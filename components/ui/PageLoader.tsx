...
import { Network, Code, Shield, Building, LucideIcon } from "lucide-react";
...
const PageLoader: React.FC<PageLoaderProps> = ({ onLoadingComplete }) => {
    ...
    const loadingSteps: { icon: LucideIcon; text: string; color: string }[] = [
        {
            icon: Network,
    ...
    return (
        <div
            className={`fixed inset-0 z-50 flex items-center justify-center bg-gradient-to-br from-primary-light/5 via-white to-accent/5 transition-all duration-800 ${isExiting ? "opacity-0 scale-110" : "opacity-100 scale-100"
                }`}
        >
            {/* Background Pattern */}
